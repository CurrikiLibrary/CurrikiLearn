...
//            'email' => 'required|email|unique:cur_users,user_email',
        ...
        if(!empty($u)) {
            ...
            // If user email exists and has the same username, add to hub.
            //$u->groups()->attach(env('APP_HUB_ID')); // Attach to hub 
            //$u->roles(env('APP_HUB_ID'))->attach(3, ['group_id' => env('APP_HUB_ID')]);    // with member role
    ...
    public function user_groups(Request $request, $user_id){
        ...
        $groups = Group::find(env('APP_HUB_ID'))->getAllChildren();

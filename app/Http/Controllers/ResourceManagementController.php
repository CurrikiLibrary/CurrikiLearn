...
class ResourceManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $groups = Group::where('parent_id', env('NASSAU_HUB_ID'))->orWhere('id', env('NASSAU_HUB_ID'))->get();
    ...
    public function update(Request $request, Resource $resource, WpApiInterface $wpApi)
    {
        ...
        $params = [
            'form_params' => [
                ...
                'groupid' => env('CURRIKI_NASSAU_GROUP_ID'),
        ...
        if ($response === false) {
            ...
            if (json_last_error() == JSON_ERROR_NONE) {
                if (isset($responseJson->resourceid)) {
                    ...
                    $res->groups()->detach();
                    //$res->groups()->attach(env('NASSAU_HUB_ID')); // All resources must belong to the hub for search purposes

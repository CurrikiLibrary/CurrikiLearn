...
class ReportsController extends Controller
{
    ...
    {	
    	...
    	$monthlyCreations = Group::find(env('NASSAU_HUB_ID'))->resources()
    		->whereBetween('contributiondate',[Carbon::now()->startOfMonth(),Carbon::now()])
    		->count();

    	$totalResources = Group::find(env('NASSAU_HUB_ID'))->resources()->count();
    	$totalUsers = Group::find(env('NASSAU_HUB_ID'))->wp_users()->count();
        ...
        $users = Group::find(env('NASSAU_HUB_ID'))->wp_users()->take(5)->get();
    ...
    private function get_monthly_views_by_district(){
        ...
        */
        // TEMP
        $groups = Group::where('parent_id', env('NASSAU_HUB_ID'))->get();
        $monthlyViewsByDistrict = ['datasets'=>[['data'=>[]]], 'labels'=>[]];

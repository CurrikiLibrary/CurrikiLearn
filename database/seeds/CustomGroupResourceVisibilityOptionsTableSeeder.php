...
class CustomGroupResourceVisibilityOptionsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        DB::table('custom_group_resource_visibility_options')->insertUsing(
            ...
            function ($query) {
                $query->select(['group_id', 'resource_id', DB::raw('3'), DB::raw('"'.Carbon::now()->format('Y-m-d H:i:s').'"'), DB::raw('"'.Carbon::now()->format('Y-m-d H:i:s').'"')])->from('custom_group_resources')->where('group_id', '=', env('APP_HUB_ID'));
            }
        );
    }
}
